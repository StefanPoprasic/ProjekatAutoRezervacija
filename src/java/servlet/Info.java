    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
      ...
      if(zapremina.isEmpty() || konjskaSnage.isEmpty() || maxBrzina.isEmpty() || ubrzanje.isEmpty() || potrosnjaKom.isEmpty()){
           request.setAttribute("Poruka","Unesite sve podatke!");
                    request.getRequestDispatcher("Informacije.jsp").forward(request, response);
      }
      else{
        try{
                    ...
                    st.executeUpdate("insert into informacije(idInfo,zapremina,konjskaSnage,maxBrzina,ubrzanje,potrosnjaKom,tekst)"
                            + " VALUES('"+idInfo+"','"+zapremina+"','"+konjskaSnage+"','"+maxBrzina+"','"+ubrzanje+"','"+potrosnjaKom+"','"+tekst+"');");
                    st.executeUpdate("update automobil set idAuto='"+idInfo+"',idInfo='"+idInfo+"'where idAuto='"+idInfo+"'");
                   
                    st.close();
                    con.close();
                    
                    request.setAttribute("Poruka","Uspesno dodat automobil!");
