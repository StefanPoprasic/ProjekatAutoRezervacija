 ...
 * @author POPRASIC
 */
public class IzmeniAutomobil extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet IzmeniAutomobil</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet IzmeniAutomobil at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(idAuto!=null && idAuto.length()>0 && marka!=null && marka.length()>0 && gorivo!=null && gorivo.length()>0 && karoserija!=null && karoserija.length()>0 && cena!=null && cena.length()>0 && serija!=null && serija.length()>0  ){
            try{
                ...
                request.getRequestDispatcher("PregledAutomobila").forward(request, response);
